...
import { ReadByID, updateProduct } from "../../backend-APIservices/crudServices";
...
const Update = (props) => {
  ...
  const updateData = () => {
    ...
    if (isEmpty(pName)) {
      ...
      updateProduct(props.id ,pName, pPrice, qty, image, uPrice, total).then((result) => {
    ...
    useEffect(() => {
        ReadByID(props.id).then((result) => {
            ...
            productPrice.value = result[0]['productPrice']
